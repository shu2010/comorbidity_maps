...
# Architecture
# ────────────
#   Each SUBJECT is a graph node.  Edges connect the k most omics-similar
#   subjects (patient-similarity network, PSN).
#
#   2-layer GCN:
...
#' build_patient_graph
#'
#' Constructs a k-NN patient-similarity network from a feature matrix.
#' Each subject is a node; directed edges connect each node to its
#' k nearest neighbours by cosine similarity, then symmetrised.
#'
#' @param X          Numeric matrix [n × d].  Rows = subjects.
#' @param k          Integer. Number of nearest neighbours per subject.
#' @param self_loops Logical. Add self-loops (handled separately in
#'                   \code{normalize_adjacency}; set FALSE here).
#' @return Symmetric binary adjacency matrix [n × n].

build_patient_graph <- function(X, k = 15, self_loops = FALSE) {
  ...
  # kNN: for each row, keep top-k off-diagonal entries
  A <- matrix(0L, n, n, dimnames = list(rownames(X), rownames(X)))
  for (i in seq_len(n)) {
    s_i         <- S[i, ]
...
train_gcn <- function(
  ...
  cat(sprintf("  Graph: %d nodes, %d edges (mean degree %.1f)\n",
              n, sum(A_raw) / 2, mean(rowSums(A_raw))))
  ...
  Y_hat_final <- final_cache$Y_hat
  rownames(Y_hat_final) <- omics_list$subject_ids
  colnames(Y_hat_final) <- colnames(Y)
...
plot_patient_graph <- function(gcn_model,
  ...
  v_names    <- V(g_sub)$name
  from_idx   <- match(edge_list$from, v_names)
  to_idx     <- match(edge_list$to,   v_names)
  edge_df    <- data.frame(
...
pdf("~/Documents/Jobs_2026/UCL/comorbidity_multiomics_GCN.pdf", width = 12, height = 8, useDingbats = FALSE)
